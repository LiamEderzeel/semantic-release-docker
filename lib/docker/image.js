...
export class Image {
  constructor(opts) {
    ...
    if (!name || typeof name !== "string") {
      const error = new TypeError(
        'Docker Image "name" is required and must be a string',
      );
      throw error;
    }
  ...
  get build_cmd() {
    return this.opts.platform.length ? this.buildx_cmd : this.docker_cmd;
  }
  get docker_cmd() {
    return [
      "build",
  ...
  async run(cmd) {
    ...
    const stream = execa(
      "docker",
      ["run", "--rm", this.name, ...array.toArray(cmd)],
      { all: true },
    );
